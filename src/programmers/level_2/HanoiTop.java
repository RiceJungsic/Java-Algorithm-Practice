...
//    }
    ...
    public void hanoiTop(int numOfDisks, char from, char by, char to) {
        if (numOfDisks == 1) {
            System.out.println("원판 1을 " + from + " 에서 " + to + " 로 이동\n");
        }
        else {
            hanoiTop(numOfDisks - 1, from, to, by);
            System.out.println("원판 " + numOfDisks + "을(를) " + from + " 에서 " + to + " 로 이동\n");
            hanoiTop(numOfDisks - 1, by, from, to);
        }
    }
